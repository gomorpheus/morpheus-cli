...
class Morpheus::StorageProvidersInterface < Morpheus::APIClient
  ...
  def list_files(id, file_path, params={})
    ...
    url = "#{@base_url}/api/storage/buckets/#{URI.encode_www_form(id.to_s)}" + "/files/#{URI.encode_www_form(file_path)}".squeeze('/')
    headers = { params: params, authorization: "Bearer #{@access_token}" }
    opts = {method: :get, url: url, headers: headers}
    execute(opts)
  end
  ...
  def upload_file(id, local_file, destination, params={})
    ...
    # end
    url = "#{@base_url}/api/storage/buckets/#{URI.encode_www_form(id.to_s)}" + "/files/#{safe_dirname}".squeeze('/')
  ...
  def download_file(id, file_path, params={})
    ...
    escaped_file_path = file_path.split("/").collect {|it| URI.encode_www_form(it) }.join("/")
    url = "#{@base_url}/api/storage/buckets/#{URI.encode_www_form(id.to_s)}" + "/download-file/#{escaped_file_path}".squeeze('/')
    headers = { params: params, authorization: "Bearer #{@access_token}" }
    opts = {method: :get, url: url, headers: headers}
    execute(opts, false)
  end
  ...
  def download_zip_chunked(id, outfile, params={})
    #url = "#{@base_url}/api/storage/buckets/#{URI.encode_www_form(id.to_s)}" + ".zip"
    url = "#{@base_url}/api/storage/buckets/#{URI.encode_www_form(id.to_s)}/download-zip"
    headers = { params: params, authorization: "Bearer #{@access_token}" }
    opts = {method: :get, url: url, headers: headers}
    # execute(opts, false)
    if Dir.exists?(outfile)
