...
class Morpheus::Cli::ArchivesCommand
  ...
  def get_file(args)
    ...
    begin
      ...
      private_download_url = "#{@appliance_url}/api/archives/download/#{URI.encode_www_form(bucket_id)}" + "/#{URI.encode_www_form(archive_file['filePath'])}".squeeze('/')
      public_download_url = nil
      if archive_file['archiveBucket'] && archive_file['archiveBucket']['isPublic']
        public_download_url = "#{@appliance_url}/public-archives/download/#{URI.encode_www_form(bucket_id)}" + "/#{URI.encode_www_form(archive_file['filePath'])}".squeeze('/')
      end
      print cyan
