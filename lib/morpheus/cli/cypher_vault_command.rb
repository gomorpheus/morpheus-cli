...
class Morpheus::Cli::CypherVaultCommand
  ...
  def connect(opts)
    ...
    # @cypher_vault_interface = @api_client.cypher
    @cypher_vault_interface = @api_client.cypher_vault
  end
  ...
  def list(args)
    ...
    begin
      ...
      if options[:dry_run]
        print_dry_run @cypher_vault_interface.dry.list(item_key, params, options)
        return 0
      end
      json_response = @cypher_vault_interface.list(item_key, params, options)
      if options[:json]
  ...
  def get(args)
    ...
    begin
      ...
      json_response = @cypher_vault_interface.get(item_key, params, options)
      # json_response = nil
      # begin
      #   json_response = @cypher_vault_interface.get(item_key, params, options)
      # rescue RestClient::Exception => e
      #   if e.response && e.response.code == 404
      #     print_red_alert "Cypher not found by key #{item_key}"
      #     # return 1
  ...
  def put(args)
    ...
    optparse = Morpheus::Cli::OptionParser.new do |opts|
      ...
      # end
      build_common_options(opts, options, [:auto_confirm, :options, :payload, :json, :dry_run, :quiet, :remote, :header, :timeout])
    ...
    begin
      ...
      if options[:dry_run]
        print_dry_run @cypher_vault_interface.dry.create(item_key, payload, options)
        return
      end
      ...
      json_response = @cypher_vault_interface.create(item_key, payload, options)
      if options[:json]
        puts as_json(json_response, options)
      elsif !options[:quiet]
  ...
  def remove(args)
    ...
    optparse = Morpheus::Cli::OptionParser.new do |opts|
      opts.banner = subcommand_usage("[key]")
      build_common_options(opts, options, [:auto_confirm, :json, :dry_run, :quiet, :remote, :header, :timeout])
    ...
    begin
      ...
      if options[:dry_run]
        print_dry_run @cypher_vault_interface.dry.destroy(item_key, params, options)
        return
      end
      json_response = @cypher_vault_interface.destroy(item_key, params, options)
      if options[:json]
        puts as_json(json_response, options)
      elsif !options[:quiet]
  ...
  def find_cypher_by_key(key, params={})
    begin
      json_response = @cypher_vault_interface.get(key, params)
